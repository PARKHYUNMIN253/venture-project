...
namespace BizOneShot.Light.Dao.Repositories
{
    public interface IScCompMappingRepository : IRepository<ScCompMapping>
    {
        ...
        Task<IPagedList<RptStatusView>> GetPagedListCompMappingInfoAync(int bizWorkSn, string mentorId, string status, int year, int page, int pageSize);
        Task<IPagedList<RptStatusView>> GetPagedListCompMappingInfoAyncForMng(int bizWorkSn, string status, int year, int page, int pageSize);

        Task<IPagedList<RptStatusView>> GetPagedListCompMappingInfoAync(int bizWorkSn, string mentorId, string status, int page, int pageSize);
    ...
    public class ScCompMappingRepository : RepositoryBase<ScCompMapping>, IScCompMappingRepository
    {
        ...
        public async Task<IPagedList<RptStatusView>> GetPagedListCompMappingInfoAync(int bizWorkSn, string mentorId, string status, int year,
    int page, int pageSize)
        {
            return await DbContext.RptStatusViews
                .Where(scm => scm.MentorId == mentorId)
                .Where(scm => scm.BizWorkSn == bizWorkSn)
                //.Where(scm => scm.CompSn == compSn)
                //.Where(scm => scm.BasicYear == year)
                //.OrderByDescending(sc => sc.Status)
                .ToPagedListAsync(page, pageSize);
        }

        public async Task<IPagedList<RptStatusView>> GetPagedListCompMappingInfoAyncForMng(int bizWorkSn, string status, int year,
            int page, int pageSize)
        {
            return await DbContext.RptStatusViews
                .Where(scm => scm.BizWorkSn == bizWorkSn)
                //.Where(scm => scm.BasicYear == year)
                .OrderByDescending(sc => sc.QuesStatus)
                .ToPagedListAsync(page, pageSize);
        }

        public async Task<IPagedList<RptStatusView>> GetPagedListCompMappingInfoAync(int bizWorkSn, string mentorId, string status,
   int page, int pageSize)
        {
            return await DbContext.RptStatusViews
                //.Where(scm => scm.MentorId == mentorId)
                ...
                //.Where(scm => scm.CompSn == compSn)
                //.Where(scm => scm.BasicYear == year)
                //.OrderByDescending(sc => sc.MentorId)
                .OrderBy(sc => sc.MentorId)
                .ToPagedListAsync(page, pageSize);
        }
